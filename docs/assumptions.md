...
This document exists to make implicit choices explicit. ChaOS treats context as a dependency, so assumptions must be visible.
...
5. Agents must be designed as bounded system components, not open-ended personalities.
...
8. The repository must support both human builders and coding agents.
9. The Meta-Agent must begin as an advisory reviewer before it can generate patches.
...
- How do project-specific forks report improvements back to ChaOS?
...
Assumptions must be reviewed when a future project inherits ChaOS and finds a gap. Repeated project-level gaps may indicate a ChaOS-level improvement.
